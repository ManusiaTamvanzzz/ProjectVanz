...
function title() {
      console.clear()
	  console.log(chalk.bold.green(figlet.textSync('ProjectVanz', {
		font: 'Standard',
...
function nocache(module, cb = () => { }) {
	console.log(chalk.black(chalk.bgGreen('Module')) + chalk.cyanBright(` ${module} sedang menantau perubahan `))
...
async function startVanz() {
    ...
    if (usePairingCode && !conn.authState.creds.registered) {
        const phoneNumber = await question('\n\nmasukan nomor telepon kamu yang awali 62xxx:\n');
	...
	require('./lib/myfunc')
	require('./message/msg')
	nocache('./message/help', module => console.log(chalk.redBright('=> Update') + chalk.redBright(` "${module}" Telah diupdate!!! `) + time ))
	nocache('./lib/myfunc', module => console.log(chalk.redBright('=> Update') + chalk.redBright(` "${module}" Telah diupdate!!! `) + time ))
	nocache('./message/msg', module => console.log(chalk.redBright('=> Update') + chalk.redBright(` "${module}" Telah diupdate!!! `) + time ))
    ...
    conn.ev.on('creds.update', saveCreds)
        ...
        conn.ev.on('group-participants.update', async (data) => {
          ...
          conn.sendMessage(data.id, { 
                text: `Welcome @${i.split("@")[0]} di  ${metadata.subject}\n\nJangan lupa baca deskripsi\n${metadata.desc !== undefined ? metadata.desc : 'Tidak Ada Deskripsi' }`,
                contextInfo: { 
                  mentionedJid: [i],
                  externalAdReply: { 
